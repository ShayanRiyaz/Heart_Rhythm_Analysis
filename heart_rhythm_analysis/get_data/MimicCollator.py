    ...
    def scan_waveform_directory(self,df_categories):
        ...
        for curr_record in matching_records:
            ...
            for pos in top_k_indices:
                ...
                file_header = wfdb.rdheader(record_name=curr_record_name, pn_dir=curr_pn_dir)
                sig_names = [name.lower() for name in file_header.sig_name]

                arterial_signals = [lbl.lower() for lbl in self.Arterial_blood_pressure_labels if lbl.lower() in sig_names]
                max_duration = file_header.sig_len
                max_freq = file_header.fs
